...
Let us view an important related result (which for brevity we shall not prove):
\begin{thm}
...
\end{equation*}
\end{thm}
What this theorem says is that a function $f$ is (pointwise) continuous at $x_0$ if and only if for all nets convergent to $x_0$, their image converges to $f(x_0)$.\\

Then relative to this, we can call oscillation as the "worst case $\delta$-net", which if it converges to zero, then all other nets must too converge to the function value (follows from completeness). We can manipulate the oscillation criterion further:
\begin{equation*}
...
The definition of Riemann and Riemann-Stieltjes integrability are fundamentally based on $\delta$-nets and (equivalently or non-equivalently depending on the conditions) nets of partitions. We examine both varieties of integrals here as well as total variation.
...
\end{equation*}
We furthermore say $\Pi_1$ is a refinement of or is finer than $\Pi_0$ if $\Pi_0 \subseteq \Pi_1$. The following lemma will prove to be quite useful:
\begin{lem}
If $\Pi_1 \supseteq \Pi_1$, then $||\Pi_1|| \leqslant ||\Pi_0||$.
\end{lem}
\begin{proof}
...
\end{proof}
The idea of the mesh will be important in definiting (Willard) subnets where the base directed set is $(\Pi, \supseteq)$ and the function $f$ is the mesh function. 
