...
We begin with limits and continuity. For some function $f: \mathbb{R} \xrightarrow{} \mathbb{R}$, we say that $L$ is the limit of $f$ at $x_0$ if:
\begin{equation*}
\forall \epsilon > 0, \exists \delta_0 > 0, \forall x \in B(x_0, \delta_0): f(x) \in B(L, \epsilon)
\end{equation*}
Notice that $\forall \delta \leqslant \delta_0: B(x_0, \delta) \subseteq B(x_0, \delta_0)$, so we can additionally say:
\begin{equation*}
\forall \epsilon > 0, \exists \delta_0 > 0, \forall \delta \leqslant \delta_0, \forall x \in B(x_0, \delta): f(x) \in B(L, \epsilon)
\end{equation*}
\pagebreak
\section{Compactness}
