...
\end{equation*}
\end{defn}
In order to show that this definition is inherently based on nets, we must show the following:
\begin{lem}
The set $P$ of all partitions of $[a, b] \subseteq \mathbb{R}$ is a directed set $(P, \supseteq)$.
\end{lem}
\begin{proof}
By subset inclusion, reflexivity and transitivity follow.\\

We aim to show directedness. Taking any two partitions $\Pi_0, \Pi_1$, we see that $\Pi_0 \cup \Pi_1 \in P$ and:
\begin{equation*}
\Pi_1 \cup \Pi_0 \supseteq \Pi_0 \land \Pi_1 \cup \Pi_0 \supseteq \Pi_1
\end{equation*}
thus proving $P$ is directed upwards.
\end{proof}
Now consider the net $\omega: P \xrightarrow{} \mathbb{R}$ where $\omega$ is the mesh function. We will denote the net as $\{||\Pi||\}_{\Pi \in P}$ for consistency with the Nets section. Then the following theorem holds:
\begin{thm}
$f$ is RS-integrable on $[a, b]$ if and only if:
\begin{equation*}
||\Pi|| \xrightarrow{} 0 \implies S(f, g, \Pi) \xrightarrow{} \int_{a}^{b}fdg
\end{equation*}
\end{thm}
