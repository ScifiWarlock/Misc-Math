...
\end{equation*}
The converse follows from the fact that $||r(v)v|| \leqslant ||r(v)||\cdot ||v||$. This is a tighter $\epsilon$-bound than using the fact (which we can prove) that:
\begin{equation*}
||r(v)|| \leqslant \sum_{i=1}^{m}||r_i(v)|| < m\epsilon
\end{equation*}
taking $\delta$ minimum (follows from the fact that if $f$ is differentiable, then $f_i$ is differentiable). 
\end{proof}
