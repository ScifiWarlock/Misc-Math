...
\begin{lem}
The partial derivative of $f: \mathbb{R}^n \xrightarrow{} \mathbb{R}^m$ exists for some $e_j$ if and only if there exists some $r(h)L \mathbb{R} \xrightarrow{} \mathcal{M}_{m \times 1}(\mathbb{R})$ such that:
\begin{equation*}
...
\begin{equation*}
f_i(x + he_j) = f_i(x) + \frac{\partial f_i(x)}{\partial e_j}h + r_i(h)h
\end{equation*}
...
\end{bmatrix}
\end{equation*}
which tends to $0$ by definition.\\
...
\end{equation*}
The converse follows from the fact that $||Ax|| \leqslant ||A|| \cdot ||x||$. Now, we can define:
\begin{equation*}
