...
\begin{document}
\maketitle
I am writing this document as my own personal notes regarding analysis and differential equations. I am attempting to cover nets and compactness rigorously in order to precisely explore Integration and Arzela-Ascoli (and related theorems/statements), which are pertinent to the study of Ordinary Differential Equations.\\

Another topic I plan to treat thoroughly is the topic of total variation and bounded variation, which is recurrent with regards to the Riemann-Stieltjes integral. Along with that, I will add some extra notes on measure theory as well.\\
...
\end{equation*}
which is not required for a directed set. The third property (which is direction) tells us whether the net is upward directed (the left side condition) or downward directed (the right side), or both!\\
...
\end{equation*}
thus proving the converse. For limits of a function at a point, the theorem is the same, just with $f(x_0)$ replaced by $L$ along with the use of punctured balls.
\end{proof}
...
\end{equation*}
where below the limit is the convergence of the $\delta$-net and the entire operation represents convergence of the image. A similar theorem can be stated for differentiability.\\
...
\section{Compactness}
In the previous section, we discussed notions of convergence for sequences and nets. We now discuss these notions for sets. Let us view the following theorem:
...
The converse of this statement only holds for metric spaces (specifically, finite dimensional metric spaces as we will see later). We first need the following lemma:
\begin{lem}
Any totally bounded metric space $X$ is separable (contains countable dense subset). 
\end{lem}
\begin{proof}
If $X$ is totally bounded, then we have:
\begin{equation*}
\forall \epsilon > 0, \exists \{x_0, \cdots, x_m\} \in X: X \subseteq \bigcup_{i = 0}^{m}B(x_i, \epsilon)
\end{equation*}
so we can say:
\begin{equation*}
\forall n \in \mathbb{N}, \exists \{x_0^{(n)}, \cdots, x_{m(n)}^{(n)}\} \in X: X \subseteq \bigcup_{i = 0}^{m}B\left(x_i, \frac{1}{n+1}\right)
\end{equation*}
Now consider the following set:
\begin{equation*}
S \coloneq \bigcup_{n \in \mathbb{N}}\{x_0^{(n)}, \cdots, x_{m(n)}^{(n)}\}
\end{equation*}
which is countably infinite. We aim to show that $X \subseteq \overline{S}$ (the reverse conclusion is trivial). Consider an arbitrary $x \in X$, then for some arbitrary $\epsilon > 0$, we have that:
\begin{equation*}
\exists n \in \mathbb{N}: \frac{1}{n+1} < \epsilon
\end{equation*}
Then, for a radius $\frac{1}{n+1}$, there exists some $x_i^{(n)}$ such that:
\begin{equation*}
\varrho(x, x_i^{(n)}) < \frac{1}{n+1} < \epsilon \implies B(x, \epsilon) \cap S \neq \varnothing
\end{equation*}
and $x_i^{(n)} \in S$, showing that each point in $x$ is adherent to $S$. Observe we used the first countability of a metric space here.
\end{proof}

\begin{lem}
Given a separable metric space $X$, any open cover contains a countable subcover.
\end{lem}
\begin{proof}
Consider any open cover $\{O_\alpha : \alpha \in I\}$ of $X$ and a countable, dense subset $S$. We have that:
\begin{equation*}
S \coloneq \{x_n : n \in \mathbb{N}\}
\end{equation*}
\begin{equation*}
\forall n \in \mathbb{N}, \exists \alpha \in I: x_n \in O_\alpha
\end{equation*}
We denote each of these $O_\alpha$ open subsets as $O_{\alpha_n}$. Since $O_\alpha$ is open, we can say there exists some $k$ such that $B\left(x_n, \frac{1}{k+1}\right) \subseteq O_\alpha$. We can define the following:
\begin{equation*}
k(n) \coloneq \inf\left\{k \in \mathbb{N}: B\left(x_n, \frac{1}{k+1}\right) \subseteq O_{\alpha_n}\right\}
\end{equation*}
Now consider an arbitrary $x \in X$. We now aim to show:
\begin{equation*}
X = \bigcup_{n \in \mathbb{N}}O_{\alpha_n}
\end{equation*}
Consider any arbitrary $x \in X$, then by the fact that $x \in \overline{S}$:
\begin{equation*}
\forall \epsilon > 0: B(x, \epsilon) \cap S \neq \varnothing
\end{equation*}
and
\begin{equation*}
\exists \alpha \in I, \exists k \in \mathbb{N}: B\left(x, \frac{1}{k+1}\right) \subseteq O_\alpha
\end{equation*}
and by density, we have that $\exists x_n \in S: x_n \in B\left(x, \frac{1}{2(k+1)}\right)$, so for any $z \in B\left(x_n, \frac{1}{2(k+1)}\right)$:
\begin{equation*}
\varrho(z, x) \leqslant \varrho(z, x_n) + \varrho(x_n, x) < \frac{1}{k+1}
\end{equation*}
thus:
\begin{equation*}
B\left(x_n, \frac{1}{2(k+1)}\right) \subseteq B\left(x, \frac{1}{k+1}\right) \subseteq O_\alpha
\end{equation*}
and we have that $x$ is in the open ball around $x_n$, thus:
\begin{equation*}
x \in B\left(x_n, \frac{1}{k(n)+1}\right) \subseteq O_{\alpha_n}
\end{equation*}
\end{proof}
%I am still unsure about this proof tbh%

\begin{lem}
If a metric space $X$ is limit point compact, then each countable open cover has a finite subcover.
\end{lem}
\begin{proof}
Assume there is a countable open cover with no finite subcover, which we will denote $\{O_n : n \in \mathbb{N}\}$ and define:
\begin{equation*}
C_n \coloneq X \setminus \bigcup_{i=0}^nO_i
\end{equation*}
Then we let $E$ be the infinite set $\{x_n : (\forall n \in \mathbb{N}: x_n \in C_n)\}$. Assume for the sake of contradiction that $E$ has a limit point $x \in X$, which tells us that:
\begin{equation*}
\exists j \in \mathbb{N}: x \in O_j
\end{equation*}
Now for all $n \geqslant j$, we have that:
\begin{equation*}
x_n \in X \setminus \bigcup_{i= 0}^n O_i \subseteq X \setminus O_j \implies x_n \notin O_j
\end{equation*}
This means only a finite number of points $\{x_0, \cdots x_{j-1}\}$ might be in $O_j$. By the $T_1$ property, we can find some $\epsilon$-neighborhood such that:
\begin{equation*}
B(x, \epsilon) \cap E \setminus \{x\} = \varnothing
\end{equation*}
or in other words, $x$ would not be an $\omega$-limit point.
\end{proof}
\pagebreak
...
\end{equation*}
Then, we say that $\Pi_1 \supseteq \Pi_0$ if and only if $\{t_i: i \in [0, n]\}_{\Pi_1} \supseteq \{t_i : i \in [0, m]\}_{\Pi_0}$. Note that this is not antisymmetric in the sense that a bi-directional relation does not imply the tagged partitions are the same. It only implies the underlying endpoints are the same. (This ordering relation independent of marked points is as per Apostol's formulation.)\\
...
\end{equation*}
which to put into words means, for all neighborhoods of $\int_{a}^{b}fdg$, there is a subnet contained in that neighborhood. By Lemma 0.5, this implies $\int_{a}^{b}fdg$ is a cluster point of the net $\{S(f, g, \Pi)\}_{\Pi \in P}$, but the converse is not necessarily true.
...
\end{thm}
\begin{proof}
If we consider the directed set $(P, \supseteq)$, then Definition 3.2 is a convergence conditon for this directed set, and the implication in Theorem 3.1 is apparent from Lemma 3.1.\\
\end{proof}
A necessary condition to Definition 3.2 and thereby Definition 3.1 would be the following:
\begin{equation*}
\forall \epsilon > 0, \forall \Pi_0 \in P, \exists \Pi \leq \Pi_0: S(f, g, \Pi) \in B\left(\int_{a}^{b}fdg, \epsilon\right)
\end{equation*}
%is this really equivalent? the forward direction is true, but the converse?
%lets take coarsest partition and set Pi_epsilon as the first partition in the epsilon ball
%then for all refinements, we can represent them as Pi cup Pi_epsilon

%lets try by contradiction. Assume the first is true and the second is not
%there exists some Pi_0 in P such that for all Pi <= Pi_0, S(f, g, Pi) in epsilon ball
%then take Pi_0 cup Pi_epsilon which is <= Pi_0 so it should not be in the epsilon ball
%but by definition 1 it is. Contradiction, possibly... need to verify more
...
We define two nets. Let $\{U(f, g, \Pi)\}_{\Pi \in P}$ be the upper-sum net and $\{L(f, g, \Pi)\}_{\Pi \in P}$ be the lower-sum net, where:
\begin{equation*}
\forall \Pi \in P: U(f, g, \Pi) \coloneq \sum_{i=1}^{n}\left(\sup_{x \in [t_{i-1}, t_i]}f(x)\right) (g(t_i) - g(t_{i-1}))
\end{equation*}
\begin{equation*}
\forall \Pi \in P: L(f, g, \Pi) \coloneq \sum_{i=1}^{n}\left(\inf_{x \in [t_{i-1}, t_i]}f(x)\right) (g(t_i) - g(t_{i-1}))
\end{equation*}
where we define $P$ as the directed set $(P, \supseteq)$, for which the reason will become apparent later on. Notice that now we do not have to worry about marked points in the partition.\\

One of the downfalls of this Darboux formulation is that it requires the least upper bound property on a totally ordered field, so extension to $\mathbb{R}^n$ does not hold unlike the Riemann integral. We now say the following:
\begin{defn}
Given the following limits:
\begin{equation*}
U(f, g, \Pi) \xrightarrow{} \overline{\int_{a}^{b}}f dg
\end{equation*}
\begin{equation*}
L(f, g, \Pi) \xrightarrow{} \underline{\int_{a}^{b}}f dg
\end{equation*}
then $f$ is Darboux-Stieltjes integrable on $[a, b]$ with respect to $g$ if:
\begin{equation*}
\overline{\int_{a}^{b}}fdg = \underline{\int_{a}^{b}}fdg
\end{equation*}
\end{defn}
We call the limit of the upper-sum nets as the upper integral, and the limit of the lower-sum nets as the lower integral. Now this definition is quite cumbersome to work with as it would require us to figure out what the upper and lower-sum nets converge to.\\

\begin{lem}
If $f$ is bounded on $[a, b]$, then the upper and lower integrals always exist, and:
\begin{equation*}
\overline{\int_{a}^{b}}fdg \coloneq \inf\{U(f, g, \Pi) : \Pi \in P\}
\end{equation*}
\begin{equation*}
\underline{\int_{a}^{b}}fdg \coloneq \sup\{L(f, g, \Pi) : \Pi \in P\}
\end{equation*}
\end{lem}
\begin{proof}
We note that if $f$ is bounded on $[a, b]$, then:
\begin{equation*}
\exists M \in \mathbb{R}, \forall x \in [a, b]: -M \leqslant f(x) \leqslant M
\end{equation*}
so, we see that the set of all upper-sums is bounded below by $-M(g(b)-g(a))$ and the set of all lower-sums is bounded above by $M(g(b)-g(a))$. We now prove the following:
\begin{equation*}
\Pi_1 \supseteq \Pi_0 \implies U(f, g, \Pi_1) \leqslant U(f, g, \Pi_0)
\end{equation*}

\end{proof}

\begin{lem}
If $f$ is bounded on $[a, b]$, then the upper and lower integrals must exist.
\end{lem}
\begin{proof}
If $f$ is bounded on the interval, then $\exists M \in \mathbb{R}, \forall x \in [a, b]: -M \leqslant f(x) \leqslant M$, thus by least upper bound properties and monotonicity of the supremum and infimum, the set of upper-sums and set of lower-sums is not empty. 
\end{proof}

\subsection{Fundamental Theorems of Calculus}
\subsection{p-Variation and Young's Inequality*}

\subsection{Henstock-Kurzweil Generalization*}
\pagebreak
\section{Measure Theory*}
In this section, we build upon the notion of outer measure of $\mathbb{R}$ introduced in the Math 131BH notes, which we used to analyze the Cantor set and apply the Lebesgue integrability criterion.

\subsection{$F_\sigma$ and $G_\delta$ sets}

We prove the criterion as follows and we shall discuss some other important properties of outer measure in $\mathbb{R}$ before transitioning to general measure spaces.
\begin{thm}
A function $f: \mathbb{R} \xrightarrow{} \mathbb{R}$ is Riemann integrable if and only if the outer measure of the set of its discontinuities is $0$.
\end{thm}
We defined outer measure in $\mathbb{R}$ as the following. Given some set $A \subseteq \mathbb{R}$:
\begin{equation*}
|A| \coloneq \inf\left\{\sum_{k \in \mathbb{N}} |I_k| : A \subseteq \bigcup_{k \in \mathbb{N}}I_k\right\}
\end{equation*}
Outer measure is properly defined for all subsets of $\mathbb{R}$ including $\mathbb{R}$ itself. If the set:
\begin{equation*}
\left\{\sum_{k \in \mathbb{N}}|I_k| : A \subseteq \bigcup_{k \in \mathbb{N}}I_k\right\}
\end{equation*}
is empty, then we get that $\inf(\varnothing) = + \infty$ if we consider the extended reals. If it is otherwise nonempty, then $0$ serves as a lower bound for all elements in the set, which means there exists an infimum.
\pagebreak
