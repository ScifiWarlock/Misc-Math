...
\end{proof}
When we refer to a partition in the next subsection, we will worked with marked partitions containing sampling points in each interval $t_i^*$. So when we say $\Pi$ is a partition of $[a, b]$, we mean that:
\begin{equation*}
\Pi \coloneq \{\{t_i\}_{i \in [0, n]}, \{t_i^*\}_{i \in [1, n]}\}
\end{equation*}
However, this does not affect the definition of mesh given in the previous section. Additionally, we can exclude the marked points from affecting subset inclusion, so $\Pi_1 \supseteq \Pi_0$ is true if $\Pi_1$ has all the interval points that $\Pi_0$ has no matter the difference in marked points.\\

We make this distinction as it ensures that the net we introduce is a function (otherwise one partition may have two different Riemann-Stieltjes sums) due to different marked points.
...
\begin{equation*}
\forall \epsilon > 0, \exists \Pi_\epsilon, \forall \Pi \leq \Pi_\epsilon: S(f, g, \Pi) \in B\left(\int_{a}^{b}fdg, \epsilon\right)
\end{equation*}
...
\subsection{Equivalence of Criteria}
