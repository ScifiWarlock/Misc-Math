...
\begin{equation*}
\forall \Pi \leq \Pi_\epsilon: \left|R(f, \Pi) - \int_{a}^{b}fdx\right| \leqslant \left|R(f, \Pi) - R(f, \Pi \cup \Pi_\epsilon)\right| + \left|R(f, \Pi \cup \Pi_\epsilon) - \int_{a}^{b}fdx\right|
\end{equation*}
\begin{equation*}
\leqslant |R(f, \Pi) - R(f, \Pi \cup \Pi_\epsilon)| + \frac{\epsilon}{2}
\end{equation*}
which follows from the fact that $\Pi \cup \Pi_\epsilon \supseteq \Pi_\epsilon$. Now using Lemma 3.3,: we observe that $\Pi \cup \Pi_\epsilon$ has all the tagged %need to fix this proof%
\begin{equation*}
\leqslant ||f|| \left|\sum_{i=1}^{n}(t_i - t_{i-1}) - \sum_{i=1}^{n+m}(t_i - t_{i-1})\right| + \frac{\epsilon}{2} \leqslant ||f|| \cdot \frac{\epsilon}{2(||f|| \cdot m + 1)} \cdot m + \frac{\epsilon}{2} \leqslant \epsilon
\end{equation*}
which follows from the fact that $\Pi \cup \Pi_\epsilon \supseteq \Pi \implies \Pi \cup \Pi_\epsilon \leq \Pi \leq \Pi_\epsilon^{(1)}$ and where $m$ is the cardinality of the set of interval endpoints in the partition $\Pi_\epsilon$. 
\end{proof}
...
\subsubsection{Riemann-Stieltjes Criteria Equivalence}
\begin{lem}
If $f$ and $g$ share a common discontinuity on $[a, b]$, then $f$ is not Riemann-Stieltjes integrable by Criterion (1).
\end{lem}
