...
\title{Notes on Analysis II\\[0.5em] \large With Solved Example Problems}
...
\end{equation*}
where $L_x$ is the limit at the point $x$. Assume $D_{1/n}$ to not be finite for some $n \in \mathbb{N}$, then we once again have a limit point $x_L$. Because $f$ only has discontinuities of the 1st kind, the left limit of $x_L$ must exist so that:
\begin{equation*}
\forall k \in \mathbb{N}, \exists \delta > 0, \forall y \in (x_L - \delta, x_L): |f(y) - f(x_L^-)| < 1/k
\end{equation*}
Choosing $k = 4n$ and using the fact that the limit at a point is an adherent point, so $L_y \in \overline{f(x_L - \delta, x_L)} \subseteq [f(x_L^-) - 1/4n, f(x_L^-) + 1/4n]$, telling us that:
\begin{equation*}
\forall y \in (x_L - \delta, x_L): |f(y) - L_y| \leqslant |f(y) - f(x_L^-)| + |L_y - f(x_L^-)| \leqslant \frac{1}{2n} < \frac{1}{n}
\end{equation*}
which is a contradiction because $x_L$ is a limit/accumulation point which means there must exist some $x_{n_k} \in D_{1/n}$. \\

So
\begin{equation*}
\forall m, n \in \mathbb{N}: S_{1/n} \text{ and } D_{1/n} \  \text{is finite}
\end{equation*}
...
% %-------------------------------------------------------------------%
% %NEED TO WORK ON THIS PROOF STILL; WILL REVISIT IT AT A LATER MOMENT%
% Interestingly, we can also solve this problem via sequences. We want to show that:
% \begin{equation*}
% \forall \{x_n\}_{n \in \mathbb{N}} \in \mathbb{R}^\mathbb{N}: (\forall n \in \mathbb{N}: x_{n+1} \leqslant x_n)
% \end{equation*}
% that if $x_n \xrightarrow{} x$ then $h(x_n) \xrightarrow{} h(x)$. We know that by the existence of the right limit:
% \begin{equation*}
% \forall x_n \in \{x_n\}_{n \in \mathbb{N}}, \forall \{x^{n}_{j}\}_{j \in \mathbb{N}} \in \mathbb{R}^\mathbb{N}: x^{n}_j \xrightarrow{} x_n \implies f(x^{n}_j) \xrightarrow{} h(x_n)
% \end{equation*}
% where $x^{n}_j$ is a non-decreasing sequence. Then:
% \begin{equation*}
% |x^{n}_j - x| \leqslant |x^{n}_j - x_n| + |x_n - x| \xrightarrow{} 0
% \end{equation*}
% To put more rigorously:
% \begin{equation*}
% \forall \epsilon > 0, \exists j_0, n_0 \geqslant 0, \forall j, n \geqslant j_0, n_0: |x^{n}_j - x| < 2\epsilon
% \end{equation*}
% So combining all terms of the $n$ sequences $\{x^{n}_j\}_{j \in \mathbb{N}}$, we find that $x^{n}_j \geqslant x$ for all $n, j \in \mathbb{N}$.\\

% Thus the sequence of values in the sequences $x^{n}_j$ converge to $x$ from the right as $j, n \xrightarrow{} \infty$, which by existence of a right limit implies that $f(x^{n}_j) \xrightarrow{} h(x)$, so:
% \begin{equation*}
% |h(x_n) - h(x)| \leqslant |h(x) - f(x^{n}_j)| + |f(x^{n}_j) - h(x_n)| \xrightarrow{} 0
% \end{equation*}
% or in $\epsilon$-terms:
% \begin{equation*}
% \forall \epsilon > 0, \exists j_0, n_0 \geqslant 0, \forall n \geqslant n_0: |h(x_n) - h(x)| < 2\epsilon
% \end{equation*}
% thus proving right continuity.
%------------------------------------------------------------------%
...
This leads us to an interesting example of a function which has a derivative BUT the derivative is not continuous. Consider:
\begin{equation*}
f(x) \coloneq \begin{cases*}
  0 \ : \ x = 0\\
...
\subsubsection{Peano's Remainder Theorem}
\begin{thm}
...
\subsubsection{Decomposition into Riemann Integral}
